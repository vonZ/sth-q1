          ...
          $.ajax({
            type: "GET",
              ...
              console.log("data: ", data); 
	            $(target).append("<div id='owl-demo' ng-init='initSlider()'></div>");
	                for (var i = 0; i < $nbrResults; i++) {
	                  if (j<20){
	                    if (data.data[j].caption === null){
	                        myCaption = '';
	                    } else {
	                        myCaption = data.data[j].caption.text;
	                    }
	                    if (data.data[j].comments.count < 2) {
	                        commentLabel = ' kommentar';
	                    } else {
	                        commentLabel = ' kommentarer';
	                    }
	                    if (data.data[j].likes.count < 2) {
	                        likeLabel = ' like'; 
	                    } else {
	                        likeLabel = ' likes';
	                    }
                      $('#owl-demo').append("<div class='item'><a href='" + data.data[j].link + "'target='_blank'><img class='img-responsive' src='" + data.data[j].images.standard_resolution.url + "' alt='" + myCaption + "'><div class='reactionContainer'><span><p><i class='fa fa-heart '></i>" + data.data[j].likes.count + likeLabel + "</p><p><i class='fa fa-comments-o'></i>" +  data.data[j].comments.count + "</i>" + commentLabel + "</p><p><i class='fa fa-clock-o'></i>" + data.data[j].created_time.timeconverter() + "</i></p></span></div></a></div>");
